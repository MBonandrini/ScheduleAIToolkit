    ...
    const ai = (() => {
        ...
        async function runOmniRoute(messages,{temperature,max_tokens,onToken}){
            ...
            try{
                ...
                const hint=location.protocol==="https:" && baseUrl.startsWith("http:")
                    ? " Your browser may be blocking an HTTP OmniRoute endpoint from this HTTPS page. If so, expose OmniRoute through an HTTPS endpoint or run the suite locally."
                    : "";
            ...
            if(onToken && content) onToken(String(content));
        ...
        async function testConnection(){
            ...
            try{
                ...
                const result=await runOmniRoute([{role:"user",content:"Reply with exactly OK"}],{temperature:0,max_tokens:5,onToken:null});
