...
function notebookRuntimeConfig(){
  try { const raw=JSON.parse(localStorage.getItem(NOTEBOOK_RUNTIME_KEY)||"{}"); return raw&&typeof raw==="object"?raw:{}; } catch (_) { return {}; }
}
...
function providerForEntry(entry){ return entry?.engine === "omniroute" ? "omniroute" : entry?.engine === "ollama" ? "ollama" : entry?.engine === "mlc" ? "mlc" : "cpu"; }
function currentProvider(){ const entry=Core?.ai?.catalog?.find(x=>x.value===Core.ai.preferred()); return providerForEntry(entry); }
function providerLabel(value){ return ({omniroute:"OmniRoute",ollama:"Ollama",cpu:"Browser CPU / WASM",mlc:"Browser WebGPU"})[value] || value; }
...
function generalView() {
  ...
  const ai = Core.ai.status(), omni = Core.ai.config(), ollama = Core.ai.ollamaConfig();
  ...
  return `<div class="grid">
    ...
    <section class="card wide"><div class="card-head">NotebookLM+ advanced configuration</div><div class="card-body">
      ${(()=>{const n=notebookRuntimeConfig();return `<div class="form-grid">
        ...
        <div class="field"><label for="nbMaxAnswer">Maximum answer tokens</label><input id="nbMaxAnswer" type="number" min="128" max="8192" step="128" value="${esc(n.maxAnswerTokens ?? 1280)}"></div>
        ...
        <div class="field"><label for="nbRequestTimeout">API / embedding timeout (seconds)</label><input id="nbRequestTimeout" type="number" min="5" max="1800" step="5" value="${esc(n.requestTimeoutSeconds ?? 300)}"></div>
        ...
        <div class="field"><label for="nbInactivity">Streaming inactivity timeout (seconds)</label><input id="nbInactivity" type="number" min="30" max="600" step="30" value="${esc(n.inactivityTimeoutSeconds ?? 180)}"></div>
        <div class="field"><label for="nbKeepAlive">Ollama keep alive</label><select id="nbKeepAlive"><option value="0" ${n.keepAlive==='0'?'selected':''}>Unload immediately</option><option value="5m" ${!n.keepAlive||n.keepAlive==='5m'?'selected':''}>5 minutes</option><option value="15m" ${n.keepAlive==='15m'?'selected':''}>15 minutes</option><option value="30m" ${n.keepAlive==='30m'?'selected':''}>30 minutes</option><option value="-1" ${n.keepAlive==='-1'?'selected':''}>Keep loaded</option></select></div>
      ...
      </div><div class="actions"><label><input id="nbSemantic" type="checkbox" ${n.semanticSearch!==false?'checked':''}> Semantic/vector retrieval when embeddings are available</label><label><input id="nbKeyword" type="checkbox" ${n.keywordSearch!==false?'checked':''}> Keyword retrieval</label><label><input id="nbRescan" type="checkbox" ${n.rescanOnOpen!==false?'checked':''}> Rescan sources when a notebook opens</label></div>`})()}
      <div class="note"><b>Unified runtime controls:</b> temperature, maximum answer tokens, reasoning/thinking, request timeout, first-response timeout, and Ollama keep-alive are also consumed by the shared suite AI runtime. The retrieval, chunking and research controls apply specifically to NotebookLM+.</div>
      <div class="actions"><button id="saveNotebookRuntime" class="btn primary" type="button">Save NotebookLM+ configuration</button><button id="resetNotebookRuntime" class="btn" type="button">Reset advanced values</button></div>
    </div></section>
    ...
    <section class="card wide"><div class="card-head">Saved connections</div><div class="card-body"><div class="rows">
      <div class="row"><span>OmniRoute URL</span><b>${esc(omni.baseUrl)}</b></div><div class="row"><span>Ollama URL</span><b>${esc(ollama.baseUrl)}</b></div><div class="row"><span>Ollama chat model</span><b>${esc(ollama.model || "Not selected")}</b></div><div class="row"><span>Ollama embedding model</span><b>${esc(Core.ai.ollamaEmbeddingModel() || "Keyword-only")}</b></div>
...
function providerSelector(selected, id){
  return `<div class="field"><label for="${id}">AI provider</label><select id="${id}">${["omniroute","ollama","cpu","mlc"].map(v=>`<option value="${v}" ${v===selected?'selected':''}>${providerLabel(v)}</option>`).join('')}</select></div>`;
}
...
function aiView() {
  ...
  const omni = Core.ai.config(), ollama = Core.ai.ollamaConfig();
  let configCard='';
  if(provider==='omniroute'){
    configCard=`<section class="card wide"><div class="card-head">OmniRoute connection</div><div class="card-body">
      <div class="form-grid"><div class="field"><label for="omniUrl">API base URL</label><input id="omniUrl" value="${esc(omni.baseUrl)}"></div><div class="field"><label for="omniKey">Endpoint API key</label><input id="omniKey" type="password" autocomplete="off" placeholder="Paste key from OmniRoute → Endpoints"></div></div>
      <div class="note">Current OmniRoute installations commonly use an endpoint key. Create/copy it in the OmniRoute dashboard under <b>Endpoints</b>. The key is held for this browser session only.</div>
      <div class="actions"><button id="saveOmni" class="btn" type="button">Save connection</button><button id="testOmni" class="btn primary" type="button">Test OmniRoute</button></div><div id="omniDiag" class="diag">Expected local API: http://localhost:20128/v1</div>
    </div></section>`;
    ...
    configCard=`<section class="card wide"><div class="card-head">Browser WebGPU</div><div class="card-body"><div class="note">No Ollama or OmniRoute service is required. The model runs in the browser using WebGPU and can consume substantial GPU memory and system RAM.</div><div class="diag">If WebGPU is unavailable or memory is limited, select Browser CPU/WASM, Ollama or OmniRoute instead.</div></div></section>`;
  }
  return `<div class="grid">
...
function tutorialContent(provider){
  ...
  if(provider==='omniroute') return `<section class="card wide"><div class="card-head">OmniRoute setup</div><div class="card-body"><ol><li>Start OmniRoute and open its dashboard at <code>http://localhost:20128</code>.</li><li>Connect at least one provider.</li><li>Open <b>Endpoints</b> and create/copy an API key if your installation requires authentication.</li><li>Use API base URL <code>http://localhost:20128/v1</code>.</li><li>Allow this site origin in OmniRoute CORS/security settings: <code>${esc(siteOrigin())}</code>.</li><li>Paste the endpoint key in Unified AI Configuration and press Test OmniRoute.</li><li>Choose <b>OmniRoute — Auto</b> or another route globally.</li></ol><div class="note">The test first calls <code>/v1/models</code>, then sends a small chat request. A 401/403 means the endpoint is reachable but the key is missing/incorrect. A browser fetch failure usually indicates CORS, local-network permission, or that OmniRoute is not running.</div></div></section>`;
  if(provider==='cpu') return `<section class="card wide"><div class="card-head">Browser CPU / WASM setup</div><div class="card-body"><ol><li>Select Browser CPU / WASM in Unified AI Configuration.</li><li>Choose the smallest model first on normal laptops.</li><li>The model downloads on first use and may take time to initialize.</li><li>No Ollama, OmniRoute, API key or local service is required.</li></ol><div class="note warning">CPU browser inference can consume several GB of RAM and may be slow. NotebookLM+ performance settings are controlled separately under General Settings.</div></div></section>`;
  return `<section class="card wide"><div class="card-head">Browser WebGPU setup</div><div class="card-body"><ol><li>Use a recent browser with WebGPU enabled.</li><li>Select Browser WebGPU in Unified AI Configuration.</li><li>Choose a model appropriate for available GPU memory.</li><li>Approve the local-model RAM/VRAM warning when loading it.</li></ol><div class="note warning">If WebGPU is unavailable or the model runs out of memory, choose a smaller model or switch to Ollama/OmniRoute.</div></div></section>`;
}
function tutorialView() {
  ...
  return `<div class="tutorial-grid"><section class="card wide emphasis"><div class="card-head">Setup Tutorial</div><div class="card-body">${providerSelector(provider,'tutorialProviderSelect')}<div class="note">Choose a provider above. The instructions below change to match that provider. The <b>top-right AI Model dropdown</b> remains the single live model selector for the entire suite.</div></div></section>${tutorialContent(provider)}
    <section class="card wide"><div class="card-head">Common troubleshooting</div><div class="card-body"><table><thead><tr><th>Issue</th><th>Check</th></tr></thead><tbody><tr><td>Local service unreachable</td><td>Service is running, correct localhost port, browser local-network permission, and CORS origin.</td></tr><tr><td>Ollama model does not support chat</td><td>Use a completion/chat model for Chat model; use embedding-only models only in the embedding field.</td></tr><tr><td>OmniRoute 401/403</td><td>Create/copy the endpoint API key in OmniRoute → Endpoints and enter it in Unified AI Configuration.</td></tr><tr><td>NotebookLM+ is slow</td><td>General Settings → NotebookLM+ performance → CPU / No GPU or Lightweight.</td></tr><tr><td>Browser model memory error</td><td>Select a smaller model or switch globally to Ollama/OmniRoute.</td></tr></tbody></table></div></section></div>`;
}
...
function bindView() {
  ...
  document.getElementById('saveNotebookRuntime')?.addEventListener('click',()=>{
    ...
    const cfg={contextTokens:num('nbContextTokens',2048,131072),topK:num('nbTopK',2,50),maxAnswerTokens:num('nbMaxAnswer',128,8192),embedBatch:num('nbEmbedBatch',1,128),workerCount:num('nbWorkers',1,8),temperature:num('nbTemperature',0,2),thinkingMode:document.getElementById('nbThinking').value,requestTimeoutSeconds:num('nbRequestTimeout',5,1800),firstResponseTimeoutSeconds:num('nbFirstResponse',30,1800),inactivityTimeoutSeconds:num('nbInactivity',30,600),keepAlive:document.getElementById('nbKeepAlive').value,semanticSearch:document.getElementById('nbSemantic').checked,keywordSearch:document.getElementById('nbKeyword').checked,chunkSize:num('nbChunkSize',500,12000),chunkOverlap:num('nbChunkOverlap',0,2000),minKeywordScore:num('nbKeywordScore',0,1),maxFileSizeMB:num('nbMaxFileSize',1,2048),researchTimeoutSeconds:num('nbResearchTimeout',5,1800),maxDiscoveryResults:num('nbDiscoveryResults',1,100),rescanOnOpen:document.getElementById('nbRescan').checked,searchEndpoint:document.getElementById('nbSearchEndpoint').value.trim(),webProxyEndpoint:document.getElementById('nbWebProxyEndpoint').value.trim(),youtubeTranscriptEndpoint:document.getElementById('nbYoutubeEndpoint').value.trim(),audioTranscriptionEndpoint:document.getElementById('nbAudioEndpoint').value.trim()};
    setStorage(NOTEBOOK_RUNTIME_KEY,JSON.stringify(cfg)); notifyPerformance(getStorage(PERF_KEY,'balanced')); setStatus('Unified AI / NotebookLM+ advanced configuration saved','ok');
  });
  document.getElementById('resetNotebookRuntime')?.addEventListener('click',()=>{try{localStorage.removeItem(NOTEBOOK_RUNTIME_KEY)}catch(_){} notifyPerformance(getStorage(PERF_KEY,'balanced')); setStatus('NotebookLM+ advanced configuration reset','ok'); render();});
  ...
  document.getElementById('clearAIConfig')?.addEventListener('click',()=>{ try{['projectControlsOmniRouteBaseUrl','projectControlsOllamaBaseUrl','projectControlsOllamaModel','projectControlsOllamaEmbeddingModel','projectControlsSharedAIModel'].forEach(k=>localStorage.removeItem(k));sessionStorage.removeItem('projectControlsOmniRouteEndpointKey');}catch(_){} Core.ai.configure({baseUrl:'http://localhost:20128/v1',endpointKey:''});Core.ai.configureOllama({baseUrl:'http://localhost:11434',model:''});Core.ai.configureOllamaEmbedding('');Core.ai.setPreferred('omniroute:auto');notifyAI();document.getElementById('resetDiag').innerHTML='<span class="ok">✓ AI settings reset.</span>';setStatus('AI settings reset','ok');});
  ...
  document.getElementById('saveOmni')?.addEventListener('click',()=>{Core.ai.configure({baseUrl:document.getElementById('omniUrl').value,endpointKey:document.getElementById('omniKey').value});document.getElementById('omniKey').value='';document.getElementById('omniDiag').innerHTML='<span class="ok">✓ OmniRoute connection saved for this session.</span>';notifyAI();setStatus('OmniRoute saved','ok');});
  document.getElementById('testOmni')?.addEventListener('click',async event=>{const button=event.currentTarget,diag=document.getElementById('omniDiag');button.disabled=true;diag.textContent='Testing OmniRoute…';try{Core.ai.configure({baseUrl:document.getElementById('omniUrl').value,endpointKey:document.getElementById('omniKey').value});const result=await Core.ai.testConnection();diag.innerHTML=`<span class="ok">✓ Connected. ${result.modelCount} route/model(s) visible; chat tested with ${esc(result.testedModel)}.</span>`;setStatus('OmniRoute connected','ok');}catch(error){diag.innerHTML=`<span class="bad">✕ ${esc(error?.message||error)}</span>`;setStatus('OmniRoute test failed','bad');}finally{document.getElementById('omniKey').value='';button.disabled=false;}});
