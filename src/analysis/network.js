...
export function drivingPredecessor(schedule, activityId) {
  ...
  return rels.map(r => ( {
    relationship: r, score: relationshipDrivingScore(schedule, r)
...
= {
  ...
  for (let i = 0; i<maxDepth; i++) {
    ...
    if (!a)break;
    chain.unshift(a);
    const drive = drivingPredecessor(schedule, id);
    if (!drive)break;
...
export function longestPath(schedule, targetId = null) {
  ...
  for (const id of topo.order) {
    const a = g.activities.get(id),
    dur = Math.max(0, Number(a.remainingDuration || a.originalDuration || 0));
    if (!dist.has(id))dist.set(id, dur);
    for (const r of g.outgoing.get(id) || []) {
      const succ = g.activities.get(r.succId),
      sd = Math.max(0, Number(succ?.remainingDuration || succ?.originalDuration || 0));
      const cand = dist.get(id) + Math.max(0, Number(r.lag || 0)) + sd;
      if (cand>(dist.get(r.succId) ?? - Infinity)) {
        dist.set(r.succId, cand);
        parent.set(r.succId, id)
      }
    }
  }
