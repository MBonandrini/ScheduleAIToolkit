    ...
    const ai = (() => {
        ...
        function model(value){
            return catalog.find(item=>item.value===value)
                || catalog.find(item=>item.value==="puter:auto");
        }
        ...
        async function loadPuter(){
            ...
            if(!puterPromise){
                puterPromise = new Promise((resolve,reject)=>{
                    ...
                    script.onerror=()=>reject(new Error("Could not load the lightweight hosted AI service."));
                    document.head.appendChild(script);
                });
            }
        ...
        async function load(value){
            ...
            if(entry.engine==="puter"){
                ...
                runtime = await transformers.pipeline(
                    "text-generation",
                    entry.id,
            ...
            try{
                localStorage.setItem("projectControlsSharedAIModel",entry.value);
            }catch(_){}
        ...
        async function ensure(value){
            const requested=value ||
                (()=>{try{return localStorage.getItem("projectControlsSharedAIModel")}catch(_){return null}})() ||
                "puter:auto";
        ...
        function extractGeneratedText(output,messages){
        ...
        async function run(messages,{
            ...
            await ensure(currentValue || "puter:auto");
            ...
            if(currentEngine==="cpu"){
                const output=await runtime(messages,{
                    max_new_tokens:max_tokens,
                    temperature,
                    do_sample:temperature>0,
                    return_full_text:false
                });
                const content=extractGeneratedText(output,messages);
                if(onToken && content) onToken(content);
                return {choices:[{message:{content}}]};
            }
