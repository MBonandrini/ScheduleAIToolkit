...
export function parseMSProjectXML(text,sourceName="schedule.xml"){
  ...
  const wbs=taskMeta.filter(x=>x.summary).map((x,index)=>{let po=parentOutline(x.outline),p=null;while(po&&!p){p=summaryByOutline.get(po)||null;po=parentOutline(po)}const last=String(x.outline||"").split(".").pop(),outlineSeq=Number(last);return {id:`WBS:${x.uid}`,parentId:p?`WBS:${p.uid}`:"",code:tag(x.block,"WBS")||x.outline||x.id,name:x.name||tag(x.block,"WBS")||x.outline||`Summary ${x.id}`,seqNum:Number.isFinite(outlineSeq)?outlineSeq:null,sourceOrder:index,raw:{uid:x.uid,outline:x.outline,summary:true}}});
