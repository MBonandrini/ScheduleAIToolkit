...
async function tx(store, mode, fn) {
  ...
  return await new Promise((resolve, reject) => {
    const t = db.transaction(store, mode), s = t.objectStore(store); let result; try {
      result = fn(s)
    } catch (e) {
      reject(e); return
    }
...
export const db = {
  ...
  async get(store, id) {
    return await new Promise(async(resolve, reject) => {
      const d = await openDB(), t = d.transaction(store), r = t.objectStore(store).get(id); r.onsuccess = () => resolve(r.result); r.onerror = () => reject(r.error)
    })
  },
  async all(store) {
    return await new Promise(async(resolve, reject) => {
      const d = await openDB(), t = d.transaction(store), r = t.objectStore(store).getAll(); r.onsuccess = () => resolve(r.result || []); r.onerror = () => reject(r.error)
    })
  },
  async del(store, id) {
