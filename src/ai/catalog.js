...
export function browserHasWebGPU(){return typeof navigator!=="undefined"&&!!navigator.gpu}
export function aiCompatibility(value){
  ...
  if((entry.engine==="mlc"||entry.engine==="gpu-transformers")&&!browserHasWebGPU())return {ok:false,reason:"WebGPU is not available in this browser. Use current Chrome/Edge with compatible graphics, or choose a CPU/WASM or Ollama model."};
  return {ok:true,reason:""};
}
export function catalogueGroups(){
  const groups=new Map();
  for(const entry of AI_CATALOG){if(!groups.has(entry.group))groups.set(entry.group,[]);groups.get(entry.group).push(entry)}
  return groups;
}
