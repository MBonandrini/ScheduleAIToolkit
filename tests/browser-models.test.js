...
export async function run(){
  ...
  for(const required of [
    "Browser CPU/WASM — Qwen2.5 0.5B","Browser CPU/WASM — Llama 3.2 1B",
    "Browser WebGPU — Qwen2.5 0.5B","Browser WebGPU — Qwen2.5 1.5B",
    "WebLLM — Qwen2.5 1.5B","WebLLM — Qwen2.5 3B","WebLLM — Qwen3 8B",
    "WebLLM — Llama 3.2 1B","WebLLM — Llama 3.2 3B","WebLLM — Phi 3.5 Mini","WebLLM — Llama 3.1 8B"
  ])assert.ok(labels.includes(required),`Missing restored model: ${required}`);

  const loaded=[];
  globalThis.__PC_AI_TEST_HOOKS__={
    transformers:{pipeline:async(task,model,options)=>{
    ...
    webllm:{CreateMLCEngine:async(model,options)=>{
      loaded.push({kind:"webllm",model});options.initProgressCallback?.({progress:.5,text:"loading"});
  ...
  try{
    for(const entry of AI_CATALOG.filter(x=>["cpu","gpu-transformers","mlc"].includes(x.engine))){
      const progress=[],r=await testBrowserAI(entry,{onProgress:x=>progress.push(x)});
    ...
    for(const entry of AI_CATALOG.filter(x=>x.engine==="cpu"))assert.ok(loaded.some(x=>x.model===entry.model&&x.device==="wasm"),`CPU route missing ${entry.model}`);
    for(const entry of AI_CATALOG.filter(x=>x.engine==="gpu-transformers"))assert.ok(loaded.some(x=>x.model===entry.model&&x.device==="webgpu"),`Transformers WebGPU route missing ${entry.model}`);
    for(const entry of AI_CATALOG.filter(x=>x.engine==="mlc"))assert.ok(loaded.some(x=>x.model===entry.model&&x.kind==="webllm"),`WebLLM route missing ${entry.model}`);
  }finally{
