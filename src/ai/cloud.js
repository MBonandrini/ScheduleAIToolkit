/**
 * Bring-your-own-key cloud adapters for Gemini, xAI/Grok, OpenAI and Anthropic.
 * Keys are read from browser-local storage and are never embedded in source files.
 */
...
export function saveCloudConfig(provider, {
  model, apiKey
}
= {
}) {
...
export function clearCloudKey(provider) {
  const s = storage();
  s?.removeItem(key(provider, "key"));
  return cloudConfig(provider)
}
function friendlyFetchError(provider, error) {
  const name = DEFAULTS[provider]?.label || provider,
...
async function request(url, options, provider) {
  ...
  try {
    ...
    throw new Error(friendlyFetchError(provider, error))
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(`${DEFAULTS[provider].label} API error: ${detail}`)
  }
  return data;
}
...
export async function cloudChat(provider, messages, {
  model = null, onProgress = null
}
= {
}) {
  const cfg = cloudConfig(provider),
  selected = model || cfg.model;
  if (!cfg.apiKey)throw new Error(`${DEFAULTS[provider].label} API key is not configured. Open Settings and paste your key first.`);
  onProgress?.( {
    title: `Contacting ${DEFAULTS[provider].label}`, detail: selected, indeterminate: true
  });
  if (provider==="gemini") {
    const {
      system,
    ...
    contents = chat.map(m => ( {
      role: m.role==="assistant"? "model": "user", parts: [ {
    ...
    if (system)body.system_instruction = {
      parts: [ {
        text: system
      }]
    };
    const data = await request(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(selected)}:generateContent`, {
      method: "POST", headers: {
    ...
    onProgress?.( {
      title: "Gemini complete", detail: selected, pct: 100
    });
    return {
      text,
      model: selected,
  ...
  if (provider==="grok") {
    const data = await request("https://api.x.ai/v1/chat/completions", {
      method: "POST", headers: {
        ...
        model: selected, messages: messages.map(m => ( {
          role: m.role, content: String(m.content || "")
        }))
      })
    }, provider);
    const text = String(data.choices?.[0]?.message?.content || "").trim();
    if (!text)throw new Error("Grok returned no text response.");
    onProgress?.( {
      title: "Grok complete", detail: selected, pct: 100
    });
    return {
      text,
      model: selected,
  ...
  if (provider==="openai") {
    ...
    const text = String(data.output_text || "").trim() || (data.output || []).flatMap(x => x.content || []).map(x => x.text || x.output_text || "").join("").trim();
    if (!text)throw new Error("OpenAI returned no text response.");
    onProgress?.( {
      title: "OpenAI complete", detail: selected, pct: 100
    });
    return {
      text,
      model: selected,
  ...
  if (provider==="anthropic") {
    ...
    const data = await request("https://api.anthropic.com/v1/messages", {
      method: "POST", headers: {
        "Content-Type": "application/json", "x-api-key": cfg.apiKey, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true"
      }, body: JSON.stringify(body)
    }, provider);
    ...
    onProgress?.( {
      title: "Claude complete", detail: selected, pct: 100
    });
    return {
      text,
      model: selected,
...
export async function testCloudAI(provider) {
  try {
    const r = await cloudChat(provider, [ {
      role: "user", content: "Reply with OK only."
    }], {
    });
    return {
      ok: true,
      message: `${DEFAULTS[provider].label} responded using ${r.model}.`,
      text: r.text
    }
  } catch (error) {
    return {
