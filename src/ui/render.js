...
export const GANTT_FIELDS={
  id:{label:"Activity ID",width:105,value:a=>a.id||""},name:{label:"Activity Name",width:260,value:a=>a.name||""},wbs:{label:"WBS",width:180,value:a=>a.wbsPath||""},status:{label:"Status",width:105,value:a=>a.status||""},
...
export function gantt(schedule,{activities=null,timescale="weekly",compression="standard",criticalOnly=false,forceRed=false,showRelationships=true,leftWidth=410,resizeKey="gantt",startDate="",endDate="",fields=null,fieldWidths={},layoutKey="wbs",barSettings={}}={}){
  ...
  const ctx={primaryResource:a=>{const x=(a.assignments||[])[0]||assignmentsByActivity.get(String(a.id||""))?.[0]||assignmentsByActivity.get(String(a.uid||""))?.[0];return x?resources.get(String(x.resourceId))||String(x.resourceId):""}};
  ...
  const groups=new Map();for(const a of acts){const st=parseDate(a.currentStart||a.start),fn=parseDate(a.currentFinish||a.finish);if((fn&&fn.getTime()<min)||(st&&st.getTime()>max))continue;const key=b.groupWbs?(a.wbsPath||"Unassigned WBS"):"All Activities";if(!groups.has(key))groups.set(key,[]);groups.get(key).push(a)}
  const grouped=[...groups.entries()].sort((a,bx)=>a[0].localeCompare(bx[0])).map(([wbs,list])=>{const starts=list.map(a=>parseDate(a.currentStart||a.start)).filter(Boolean),finishes=list.map(a=>parseDate(a.currentFinish||a.finish)).filter(Boolean),gs=starts.length?pos(new Date(Math.min(...starts.map(d=>d.getTime())))):0,gf=finishes.length?pos(new Date(Math.max(...finishes.map(d=>d.getTime())))):0;return b.groupWbs?`<details class="gantt-group" open><summary><span class="gantt-summary-fields" style="--gantt-field-template:${fieldTemplate}"><span style="grid-column:1/-1">${esc(wbs)} <small>${list.length} activities</small></span></span><span class="summary-time"><i style="left:${Math.min(gs,gf)}%;width:${Math.max(.3,Math.abs(gf-gs))}%"></i></span></summary>${list.map(renderActivity).join("")}</details>`:list.map(renderActivity).join("")}).join("");
  ...
  <div class="gantt-footnote muted">Use the field chooser to build the WBS / Activity field table, drag individual header dividers to resize columns, and use Bars, labels & display to tailor the chart. Relationship lines use orthogonal P6-style elbows.</div></div>`;
}
