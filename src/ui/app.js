...
function assessmentReport(s){
  ...
  switch(state.assessmentReport){
    ...
    case"critical":{
      const crit=s.activities.filter(a=>a.critical||a.totalFloat<=0),layout=state.ganttLayouts.critical||freshGanttLayout("critical");return `${gantt(s,{activities:crit,criticalOnly:true,forceRed:true,timescale:state.ganttTimescale,compression:state.ganttCompression,showRelationships:state.ganttRelationships,leftWidth:state.criticalLeftWidth,resizeKey:"criticalLeftWidth",startDate:state.criticalStartDate,endDate:state.criticalFinishDate,fields:layout.fields,fieldWidths:layout.widths,barSettings:layout.bars,layoutKey:"critical"})}<section class="panel"><h2>Critical / zero-float activities</h2><p class="muted">Drag the vertical dividers in the headings to resize the table columns. The Gantt's WBS/Activity divider can also be dragged wider or narrower.</p>${table(["Activity","WBS","Finish","TF"],crit.map(a=>[`${esc(a.id)} · ${esc(a.name)}`,esc(a.wbsPath),isoDate(a.currentFinish||a.finish),a.totalFloat.toFixed(1)]),{resizable:true,resizeKey:"criticalPath"})}</section>`;
    }
    case"logic":{
...
function bindGanttInteractions(schedule){
  ...
  panel.querySelectorAll('.gantt-group').forEach(d=>d.addEventListener('toggle',scheduleRedraw));g.closest('.gantt-wrap')?.addEventListener('scroll',scheduleRedraw,{passive:true});window.addEventListener('resize',scheduleRedraw,{once:true});requestAnimationFrame(drawLinks);
}
...
async function importClaimCsv(file){const rows=csvObjects(await file.text()),existing=new Map(state.claims.map(r=>[String(r.code||"").toLowerCase(),r]));let count=0;for(const row of rows){const code=String(fieldFrom(row,["ID","Code","Event ID"],`CE-${String(state.claims.length+count+1).padStart(3,"0")}`)).trim(),old=existing.get(code.toLowerCase());await saveClaim({...(old||{}),code,title:fieldFrom(row,["Event","Title","Event Title"]),date:fieldFrom(row,["Date","Event Date"],""),description:fieldFrom(row,["Description","Narrative"],""),activityIds:String(fieldFrom(row,["Activity IDs","ActivityIDs","Activities"],"")).split(/[;,]/).map(x=>x.trim()).filter(Boolean),milestoneId:fieldFrom(row,["Milestone ID","MilestoneID"],""),notice:fieldFrom(row,["Notice","Notice Ref","Correspondence"],""),instruction:fieldFrom(row,["Instruction","Instruction Ref"],"")});count++}await refreshData();return count}
function exportClaimCsv(){const rows=state.claims.map(c=>[c.code||c.id,c.title||"",c.date||"",c.description||"",(c.activityIds||[]).join(";"),c.milestoneId||"",c.notice||"",c.instruction||""]);downloadBlob(new Blob([toCSV(["ID","Event","Date","Description","Activity IDs","Milestone ID","Notice","Instruction"],rows)],{type:"text/csv"}),"claims-forensics-events.csv")}
