...
const state={
  ...
  timeMachineScheduleIds:Array(8).fill(""),scurveBasis:"activities",scurveResourceId:"",notebookTab:"chat",notebookOutputs:{},
...
function notebookChatPanel(){
  ...
  return `<section class="panel chat"><div class="messages" id="messages">${hist.map(m=>`<div class="msg ${m.role==="user"?"user":""}">${esc(m.content)}${m.sources?.length?`<div style="margin-top:7px;font-size:9px;opacity:.75">Evidence: ${m.sources.slice(0,8).map(s=>esc(s.name)).join(" · ")}${m.structuredTool?` · Tool: ${esc(m.structuredTool)}`:""}</div>`:""}</div>`).join("")||`<div class="muted">Checked Project Repository files are available as context.</div>`}</div><div class="chatbox"><select id="chatRole">${roles.map(r=>`<option ${r==="Project Controls Manager"?"selected":""}>${r}</option>`).join("")}</select><textarea id="chatInput" placeholder="Ask about the current project…"></textarea><button id="chatSend">Send</button></div></section>`;
}
function notebookReportMarkup(){
  ...
  return `<h1>Project Controls Notebook Report</h1><p>Generated ${new Date().toLocaleString()}</p><h2>Source set</h2><p>${files.length} checked repository file(s)${s?` and schedule <strong>${esc(scheduleLabel(s))}</strong>`:""}.</p>${s?`<h2>Schedule overview</h2><ul><li>Activities: ${summary.activities}</li><li>Progress: ${summary.progress.toFixed(1)}%</li><li>Forecast finish: ${esc(summary.forecastFinish||"—")}</li><li>Critical / zero float: ${summary.critical}</li><li>Negative float: ${summary.negativeFloat}</li></ul><h2>Schedule narrative</h2>${n.paragraphs.map(x=>`<p>${esc(x)}</p>`).join("")}`:`<p>No active schedule is selected.</p>`}<h2>Repository files</h2><ul>${files.map(f=>`<li>${esc(f.name)} · ${esc(f.category)} · ${Math.round((f.size||0)/1024)} KB</li>`).join("")||"<li>No checked files.</li>"}</ul>`;
}
function notebookGraphicSvg(){
  const s=activeSchedule(),sum=s?scheduleSummary(s):null,h=s?scheduleHealth(s):null,vals=sum?[sum.progress,Math.min(100,h.score),Math.min(100,sum.activities?sum.critical/sum.activities*100:0),Math.min(100,sum.activities?sum.negativeFloat/sum.activities*100:0)]:[0,0,0,0],labels=["Progress","Health","Critical %","Negative float %"],w=900,hg=360,max=100;
  const bars=vals.map((v,i)=>{const bw=130,gap=70,x=90+i*(bw+gap),bh=Math.max(2,v/max*220),y=290-bh;return `<rect x="${x}" y="${y}" width="${bw}" height="${bh}" rx="6" fill="currentColor" opacity="${.95-i*.14}"/><text x="${x+bw/2}" y="${y-10}" text-anchor="middle" fill="currentColor" font-size="18">${Number(v).toFixed(1)}%</text><text x="${x+bw/2}" y="325" text-anchor="middle" fill="currentColor" font-size="15">${esc(labels[i])}</text>`}).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${hg}" width="${w}" height="${hg}"><rect width="100%" height="100%" fill="white"/><g color="#173b57"><text x="40" y="42" font-family="Segoe UI,Arial" font-size="24" font-weight="700" fill="currentColor">Project Controls Summary</text><text x="40" y="68" font-family="Segoe UI,Arial" font-size="13" fill="currentColor">${esc(s?scheduleLabel(s):"No active schedule")}</text>${bars}</g></svg>`;
}
function notebookAudioScript(){
  const s=activeSchedule();if(!s)return "No active schedule is selected. Select a schedule to generate an audio brief.";
  const n=scheduleNarrative(s,null);return `Project controls audio brief. ${n.paragraphs.join(" ")}`;
}
function renderNotebook(){
  $("workspace").innerHTML=`${viewHead("NotebookLM+","Project notebook with chat plus downloadable report, graphics, data and audio-brief outputs.")}<div class="subtabs"><button data-notebook-tab="chat" class="${state.notebookTab==="chat"?"active":""}">Chat</button><button data-notebook-tab="outputs" class="${state.notebookTab==="outputs"?"active":""}">Outputs</button></div>${state.notebookTab==="chat"?notebookChatPanel():`<div class="grid grid2 notebook-outputs"><section class="panel"><h2>Report</h2><p class="muted">Build a deterministic project-controls report from the active schedule and checked repository files.</p><div class="actions"><button class="btn primary" id="nbPreviewReport">Preview report</button><button class="btn" id="nbDownloadReport">Download HTML</button></div><div id="nbReportPreview" class="output-preview"></div></section><section class="panel"><h2>Graphics</h2><p class="muted">Generate a downloadable SVG schedule summary graphic.</p><div class="actions"><button class="btn primary" id="nbPreviewGraphic">Preview graphic</button><button class="btn" id="nbDownloadGraphic">Download SVG</button></div><div id="nbGraphicPreview" class="output-preview"></div></section><section class="panel"><h2>Data</h2><p class="muted">Download the current internal activity model for further analysis.</p><button class="btn primary" id="nbDownloadData">Download activity CSV</button></section><section class="panel"><h2>Audio brief</h2><p class="muted">Creates an audio-ready briefing script and can read it aloud with the browser's local speech service. Browser speech APIs do not expose a reliable downloadable audio stream, so the downloadable artifact is the script.</p><div class="actions"><button class="btn primary" id="nbListenAudio">Listen</button><button class="btn" id="nbDownloadAudioScript">Download script</button></div><div class="output-preview"><pre id="nbAudioScript">${esc(notebookAudioScript())}</pre></div></section></div>`}`;
  document.querySelectorAll("[data-notebook-tab]").forEach(b=>b.onclick=()=>{state.notebookTab=b.dataset.notebookTab;renderNotebook()});
  if(state.notebookTab==="chat"){bindChat("notebook","Project Controls Manager");return}
  $("nbPreviewReport").onclick=()=>{$("nbReportPreview").innerHTML=notebookReportMarkup()};
  $("nbDownloadReport").onclick=()=>{const html=`<!doctype html><html><head><meta charset="utf-8"><title>Project Controls Notebook Report</title></head><body>${notebookReportMarkup()}</body></html>`;downloadBlob(new Blob([html],{type:"text/html"}),"notebook-project-controls-report.html")};
  $("nbPreviewGraphic").onclick=()=>{$("nbGraphicPreview").innerHTML=notebookGraphicSvg()};
  $("nbDownloadGraphic").onclick=()=>downloadBlob(new Blob([notebookGraphicSvg()],{type:"image/svg+xml"}),"notebook-schedule-summary.svg");
  $("nbDownloadData").onclick=()=>{const s=activeSchedule(),rows=(s?.activities||[]).map(a=>[a.id,a.name,a.wbsPath,a.status,isoDate(a.currentStart||a.start),isoDate(a.currentFinish||a.finish),a.originalDuration,a.remainingDuration,a.totalFloat,a.percent,a.budgetUnits,a.actualUnits,a.remainingUnits,a.budgetCost,a.actualCost,a.remainingCost]);downloadBlob(new Blob([toCSV(["ID","Activity","WBS","Status","Start","Finish","Original duration","Remaining duration","Total float","Percent","Budget units","Actual units","Remaining units","Budget cost","Actual cost","Remaining cost"],rows)],{type:"text/csv"}),"notebook-activity-data.csv")};
  $("nbListenAudio").onclick=()=>{if(!("speechSynthesis" in window)){alert("This browser does not expose speech synthesis.");return}speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(notebookAudioScript());speechSynthesis.speak(u)};
  $("nbDownloadAudioScript").onclick=()=>downloadBlob(new Blob([notebookAudioScript()],{type:"text/plain"}),"notebook-audio-brief.txt");
}
...
function dcmaVisualCard(c,index){
  const score=c.pass?100:Math.max(4,100-Math.min(96,Math.max(12,(c.rateNum/(Math.max(c.limitNum,.01)))*28))),kind=["doughnut","gauge","pie","columns","bar","area"][index%6];
  const ring=`<div class="quality-ring" style="--quality:${score}"><span>${score.toFixed(0)}</span></div>`;
  const gauge=`<svg class="quality-svg" viewBox="0 0 120 70"><path d="M15 60 A45 45 0 0 1 105 60" class="q-bg"/><path d="M15 60 A45 45 0 0 1 105 60" class="q-value" pathLength="100" stroke-dasharray="${score} 100"/><text x="60" y="58" text-anchor="middle">${score.toFixed(0)}</text></svg>`;
  const columns=`<div class="quality-columns">${[.35,.55,.75,1].map((m,i)=>`<i style="height:${Math.max(4,score*m)}%" title="${(score*m).toFixed(0)}"></i>`).join("")}</div>`;
  const bar=`<div class="quality-bar"><i style="width:${score}%"></i><span>${score.toFixed(0)}/100</span></div>`;
  const area=`<svg class="quality-svg" viewBox="0 0 120 70"><polygon points="5,62 5,${62-score*.42} 35,${62-score*.25} 68,${62-score*.55} 115,${62-score*.5} 115,62" class="q-area"/><polyline points="5,${62-score*.42} 35,${62-score*.25} 68,${62-score*.55} 115,${62-score*.5}" class="q-line"/></svg>`;
  const visual=kind==="gauge"?gauge:kind==="columns"?columns:kind==="bar"?bar:kind==="area"?area:ring;
  return `<div class="quality-card" title="${esc(c.definition)}"><div><strong>${esc(c.name)}</strong>${c.pass?badge("PASS","good"):badge("FAIL","danger")}</div>${visual}<small>Actual ${esc(c.rate)} · Guideline ${esc(c.guideline)} · ${esc(kind)}</small></div>`;
}
...
function assessmentReport(s){
  ...
  switch(state.assessmentReport){
    ...
    case"comparison":{
      ...
      return `<section class="panel"><h2>Schedule Comparison</h2>${toolbar}${projectWarning}</section><div class="metrics">${metric("Data date interval",`${comp.summary.dataDateDays}d`)}${metric("Forecast movement",`${comp.summary.forecastFinishDays>=0?"+":""}${comp.summary.forecastFinishDays}d`)}${metric("Progress movement",`${comp.summary.progressPoints>=0?"+":""}${comp.summary.progressPoints.toFixed(1)} pts`)}${metric("Added / deleted",`${comp.added.length} / ${comp.deleted.length}`)}${metric("Logic + / -",`${comp.relationshipAdded.length} / ${comp.relationshipDeleted.length}`)}${metric("Critical entered / left",`${comp.migration.entered.length} / ${comp.migration.left.length}`)}</div><section class="panel"><h2>Material Change Register</h2>${table(["Activity","Start Δ","Finish Δ","Duration Δ","Float Δ","Progress Δ","Calendar","Constraint"],changeRows.slice(0,500).map(x=>[`${esc(x.id)} · ${esc(x.name)}`,`${x.startDays>=0?"+":""}${x.startDays}d`,`${x.finishDays>=0?"+":""}${x.finishDays}d`,`${x.durationDays>=0?"+":""}${x.durationDays.toFixed(1)}d`,`${x.floatDays>=0?"+":""}${x.floatDays.toFixed(1)}d`,`${x.progressPoints>=0?"+":""}${x.progressPoints.toFixed(1)} pts`,x.calendarChanged?"Changed":"—",x.constraintChanged?"Changed":"—"]))}</section><div class="grid grid2"><section class="panel"><h2>New / deleted activities</h2>${table(["Type","Activity"],[...comp.added.slice(0,200).map(x=>["Added",`${esc(x.id)} · ${esc(x.name)}`]),...comp.deleted.slice(0,200).map(x=>["Deleted",`${esc(x.id)} · ${esc(x.name)}`])])}</section><section class="panel"><h2>Relationship changes</h2>${table(["Type","Relationship","Lag"],[...comp.relationshipAdded.slice(0,200).map(r=>["Added",`${esc(r.predId)} ${esc(r.type)} ${esc(r.succId)}`,`${r.lag}d`]),...comp.relationshipDeleted.slice(0,200).map(r=>["Deleted",`${esc(r.predId)} ${esc(r.type)} ${esc(r.succId)}`,`${r.lag}d`])])}</section></div>`;
    }
    case"week":{
    ...
    case"critical":{
      const crit=s.activities.filter(a=>a.critical||a.totalFloat<=0);return `${gantt(s,{activities:crit,criticalOnly:true,forceRed:true,timescale:state.ganttTimescale,compression:"compact",showRelationships:state.ganttRelationships,leftWidth:state.criticalLeftWidth,resizeKey:"criticalLeftWidth"})}<section class="panel"><h2>Critical / zero-float activities</h2><p class="muted">Drag the vertical dividers in the headings to resize the table columns. The Gantt's WBS/Activity divider can also be dragged wider or narrower.</p>${table(["Activity","WBS","Finish","TF"],crit.map(a=>[`${esc(a.id)} · ${esc(a.name)}`,esc(a.wbsPath),isoDate(a.currentFinish||a.finish),a.totalFloat.toFixed(1)]),{resizable:true,resizeKey:"criticalPath"})}</section>`;
    }
    case"logic":{
...
function bindGanttInteractions(schedule){
  ...
  const drawLinks=()=>{
    ...
    const ns='http://www.w3.org/2000/svg',defs=document.createElementNS(ns,'defs'),marker=document.createElementNS(ns,'marker');marker.setAttribute('id','ganttArrow');marker.setAttribute('viewBox','0 0 7 6');marker.setAttribute('refX','7');marker.setAttribute('refY','3');marker.setAttribute('markerWidth','7');marker.setAttribute('markerHeight','6');marker.setAttribute('orient','auto');const tip=document.createElementNS(ns,'path');tip.setAttribute('d','M0,0 L0,6 L7,3 z');tip.setAttribute('class','gantt-link-arrow');marker.appendChild(tip);defs.appendChild(marker);overlay.appendChild(defs);
    ...
    for(const rel of schedule.relationships||[]){
      ...
      const a=pb.getBoundingClientRect(),b=sb.getBoundingClientRect(),x1=a.right-gr.left+g.scrollLeft,y1=a.top+a.height/2-gr.top+g.scrollTop,x2=b.left-gr.left+g.scrollLeft,y2=b.top+b.height/2-gr.top+g.scrollTop,mx=x1+Math.max(18,Math.abs(x2-x1)*.42);
      const path=document.createElementNS(ns,'path');path.setAttribute('d',`M${x1.toFixed(1)} ${y1.toFixed(1)} C${mx.toFixed(1)} ${y1.toFixed(1)},${mx.toFixed(1)} ${y2.toFixed(1)},${x2.toFixed(1)} ${y2.toFixed(1)}`);path.setAttribute('class','gantt-link');path.setAttribute('marker-end','url(#ganttArrow)');const title=document.createElementNS(ns,'title');title.textContent=`${rel.predId} ${rel.type||'FS'} ${rel.succId}${Number(rel.lag||0)?` · lag ${Number(rel.lag).toFixed(1)}d`:''}`;path.appendChild(title);overlay.appendChild(path);
    }
  };
