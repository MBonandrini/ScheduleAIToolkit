import {chat as ollamaChat,ollamaConfig} from "./ollama.js?v=1.2.0";
import {browserCPU,browserTransformersGPU,browserMLC,releaseBrowserAI,testBrowserAI} from "./browser.js?v=1.2.0";
import {AI_CATALOG,DEFAULT_AI_VALUE,aiEntry,aiCompatibility,catalogueGroups} from "./catalog.js?v=1.2.0";
import {toolRegistry,inferTool} from "./tools.js?v=1.2.0";
import {selectedContext} from "../repository/repository.js?v=1.2.0";
import {scheduleSummary} from "../core/model.js?v=1.2.0";

const AI_KEY="pcai.model";
const LEGACY_KEY="pcai.engine";
...
function portfolioSummary(revisions=[]){
  return (revisions||[]).slice(-30).map(s=>{
    const x=scheduleSummary(s);
    return {
      file:s.sourceName||s.name,project:s.projectName||s.name,dataDate:x.dataDate,
      activities:x.activities,progress:Number(x.progress.toFixed(1)),
      critical:x.critical,negativeFloat:x.negativeFloat,forecastFinish:x.forecastFinish
    };
  });
}
...
export async function askAI({question,role="Project Controls Manager",current=null,previous=null,revisions=[],history=[]}){
  ...
  let structured=null;
  if(call&&reg[call.name]){try{structured=reg[call.name](call.args)}catch(e){structured={error:e.message}}}

  const repo=await selectedContext({
    question,maxFileChars:budget.repoFile,maxTotalChars:budget.repoTotal,skipScheduleText:true
  });
  const portfolio=portfolioSummary(revisions);
  const activeOverview=current?scheduleSummary(current):null;
...
${compactStructured(activeOverview,budget.portfolio)}

SCHEDULE PORTFOLIO / REVISION SUMMARY:
${compactStructured(portfolio,budget.portfolio)}
...
${repo.text}`;
  ...
  const messages=[
    ...
    ...history.slice(-budget.historyMessages).map(x=>({role:x.role,content:String(x.content||"").slice(0,3000)})),
    {role:"user",content:question}
  ];
  const onProgress=x=>progress(x);
  let out;
  if(entry.engine==="cpu"){
    out=await browserCPU(messages,{model:entry.model,dtype:entry.dtype||"q4",onProgress});
  }else if(entry.engine==="gpu-transformers"){
    out=await browserTransformersGPU(messages,{model:entry.model,dtype:entry.dtype||"q4",onProgress});
  }else if(entry.engine==="mlc"){
    out=await browserMLC(messages,{model:entry.model,onProgress});
  }else if(entry.engine==="ollama"){
    out=await ollamaChat(messages,{onProgress});
  }else{
    throw new Error("This AI option is not available yet.");
  }
  return {
    ...out,
    aiValue:entry.value,aiLabel:aiLabel(entry.value),
    sources:repo.files.map(f=>({id:f.id,name:f.name,category:f.category,relativePath:f.relativePath})),
...
export async function testSelectedAI({onProgress=null}={}){
  ...
  if(entry.engine==="ollama"){
    return {ok:true,message:"Use the Ollama Test & Save control in Settings for a full local-server test.",entry};
  }
  try{
    const r=await testBrowserAI(entry,{onProgress:onProgress||((x)=>progress(x))});
    return {ok:r.ok,message:r.ok?`${entry.label} responded correctly.`:`${entry.label} returned: ${r.text}`,entry};
  }catch(error){
    return {ok:false,message:error.message||String(error),entry};
  }
}
