...
function relevantExcerpt(text,question,maxChars){
  ...
  for(const term of terms){
    ...
    while((pos=lower.indexOf(term,pos))>=0&&hits++<8){
      const a=Math.max(0,pos-700),b=Math.min(raw.length,pos+term.length+1300);
      windows.push([a,b]);pos+=term.length;
    }
  }
...
export async function selectedContext({maxFileChars=12000,maxTotalChars=36000,question="",skipScheduleText=true}={}){
  ...
  for(const f of files){
    ...
    let blob=f.blob;
    if(!blob&&f.folderKey){const handle=folderHandles.get(f.folderKey);if(handle)blob=await resolveHandleFile(handle,f.relativePath)}
