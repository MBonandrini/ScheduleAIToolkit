/**
 * Schedule import router. Selects the correct parser/conversion path from the
 * supplied file type and returns the normalized schedule representation.
 */
import {
  parseXER
} from "./xer.js";
...
export function mppBridgeUrl() {
  return localStorage.getItem("pcai.mppBridgeUrl") || MPP_BRIDGE_DEFAULT
}
export function setMppBridgeUrl(url) {
  const clean = String(url || MPP_BRIDGE_DEFAULT).replace(/\/$/, "");
  localStorage.setItem("pcai.mppBridgeUrl", clean);
  return clean
}
export async function probeMppBridge(baseUrl = mppBridgeUrl()) {
  ...
  try {
    const r = await fetch(`${String(baseUrl).replace(/\/$/, "")}/health`, {
    ...
    return {
      ok: true,
      message: j.message || "Local MPP parser is ready",
      version: j.version || ""
    }
  } catch (error) {
    return {
      ok: false,
      message: error.name==="AbortError"? "Local MPP parser did not respond.": error.message || String(error)
    }
  } finally {
    clearTimeout(timer)
  }
...
async function parseMPPViaLocalBridge(file) {
  const base = mppBridgeUrl(),
  controller = new AbortController(),
  timer = setTimeout(() => controller.abort(), 90000);
  try {
    const body = await file.arrayBuffer(),
    r = await fetch(`${base}/convert`, {
      method: "POST", headers: {
        "Content-Type": "application/octet-stream", "X-Filename": encodeURIComponent(file.name)
      }, body, signal: controller.signal
    });
    const text = await r.text();
    if (!r.ok)throw new Error(text || `MPP converter returned HTTP ${r.status}`);
    const parsed = parseMSProjectXML(text, file.name);
    parsed.sourceFormat = "mpp";
    return parsed;
  } catch (error) {
